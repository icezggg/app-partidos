...
function getSeasonData(seasonKey) {
    if (seasonKey === 't1') return DB['TEMP 1'] || DB['Temp 1'] || DB['TEMP1'] || [];
    if (seasonKey === 't2') return DB['TEMP 2'] || DB['Temp 2'] || DB['TEMP2'] || [];
    return DB['HISTORICA'] || [];
}
...
function calculateRecords() {
    ...
    matches.forEach(m => {
        const diff = Math.abs(n(m.Goles_E1) - n(m.Goles_E2));
        const total = n(m.Goles_E1) + n(m.Goles_E2);
    ...
    const playerHistory = {};
    matches.forEach(m => {
        (matchDetailsMap[m.ID_Partido] || []).forEach(d => {
            if(!playerHistory[d.Jugador]) playerHistory[d.Jugador] = [];
            playerHistory[d.Jugador].push({ res: d.Resultado, nota: n(d.Nota) });
        });
    });
    ...
    for(const player in playerHistory) {
        ...
        let streak = 0;
        for(let i = history.length - 1; i >= 0; i--) { if(history[i].res === 'Ganó') streak++; else break; }
...
function getTeammatesAndRivals(playerName) {
    ...
    matches.forEach(m => {
        ...
        const iWon = me.Resultado === 'Ganó';
        const est = m.Estadio || 'St. Diego';
        ...
        dets.forEach(d => {
            ...
            if(theirTeam === myTeam) {
                ...
                rivals[d.Jugador].total++; if(!iWon) rivals[d.Jugador].losses++;
            }
        });
    });

    let bestMate = { name: '-', wr: 0 };
    for(const t in teammates) { if(teammates[t].total >= 2) { const wr = (teammates[t].wins / teammates[t].total) * 100; if(wr > bestMate.wr) bestMate = { name: t, wr: wr.toFixed(0) }; } }
    let worstRival = { name: '-', lr: 0 };
    for(const r in rivals) { if(rivals[r].total >= 2) { const lr = (rivals[r].losses / rivals[r].total) * 100; if(lr > worstRival.lr) worstRival = { name: r, lr: lr.toFixed(0) }; } }
...
function renderAdmin() {
    const app = document.getElementById('app-view');
    const estadios = DB['ESTADIOS'] || ['St. Diego'];
...
async function saveMatch() { 
    adminMatch.Fecha = document.getElementById('m_fecha').value; adminMatch.Estadio = document.getElementById('m_estadio').value; adminMatch.Temporada = document.getElementById('m_temp').value; adminMatch.MVP = document.getElementById('m_mvp').value; adminMatch.Goles_E1 = document.getElementById('m_g1').value; adminMatch.Goles_E2 = document.getElementById('m_g2').value; 
    if(!adminMatch.Detalle.length) return; 
    try { const res = await fetch(API_URL, { method: 'POST', body: JSON.stringify({action: 'saveMatch', ...adminMatch, Password: adminPassword}) }); const data = await res.json(); if(data.status === 'success') { alert("Partido guardado!"); adminMatch = { Fecha: '', Temporada: '2', Goles_E1: 0, Goles_E2: 0, MVP: '', Estadio: 'St. Diego', Detalle: [] }; await loadData(); showView('home'); } } catch(e) { alert("Error."); } 
}

async function saveEditedMatch() {
    if(!adminMatch.ID_Partido) return alert("Error: No se seleccionó ningún partido para editar.");
    
    // Si por algún motivo se perdió la contraseña en memoria, la volvemos a pedir
    if(!adminPassword) {
